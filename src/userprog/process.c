...
start_process(void *file_name_)
{
  ...
  cur_thread->load_done = success; // [ADDED_Lab2_parent_child]
  if (success)
  {
    /* [EDITED_Lab2_argument_passing] */
  ...
  sema_up(&cur_thread->load_sema); // process parent process -> syscall EXEC
  /* ----------------------------- */
  /* If load failed, quit. */
...
void process_exit(void)
{
  ...
  /*[ADDED_Lab2_file_descriptor]*/
  while(cur->cur_fd > 2)
  {
    close_file(cur->cur_fd);
    cur->cur_fd--;
  }
...
int add_to_fd (struct file * f, char* filename)
{
  ...
  if (cur->cur_fd >= 128)     // if cur_fd is larger than fd size, reuse fd
  {
    for(file_id=2; file_id<128; file_id++)
    {
      if(cur->file_descriptor[file_id] == NULL)
      {
        is_empty = true;
        break;
      }
    ...
    if(is_empty)
    {
      cur->file_descriptor[file_id] = f;
      cur->fd_name[file_id] = (char *)malloc(sizeof(char) * (strlen(filename) + 1));
      strlcpy(cur->fd_name[file_id], filename, strlen(filename));
      return file_id;
    }
    else { return -1;}
  }
...
struct file *get_file(int fd)
{
  ...
  f = (fd >= 2) && (fd <=cur->cur_fd) ? cur->file_descriptor[fd] : NULL;
